...
def parser():
    ...
    parser.add_argument('--batch_size', default=32, type=int, help='size of data per training iteration') #16
    parser.add_argument('--lr', default=1e-3, type=float, help='learning rate') #1e-2
    ...
    parser.add_argument('--early_stop', default=100, type=int, #30
        help='the number of epochs to stop training if the loss is not decrease') #5
    parser.add_argument('--early_stop_pretrain', default=30, type=int, help='early stop for pretrain') #5
...
def print_args(args, logger=None):
    ...
    #     else:
    #         print('{:<16} : {}'.format(k, v))
    vari = vars(args).items()
    for i, j in vari:
        if logger is not None:
            logger.info('{:<16} : {}'.format(i, j))
        else:
            print('{:<16} : {}'.format(i, j))
